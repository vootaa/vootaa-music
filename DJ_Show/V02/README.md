...
- 8 字型锚点：使用 Lissajous 或柯西姆 8 字曲线（例如 x=sin(αt)、y=sin(βt+δ)；或 Bernoulli lemniscate）在两参数间联动（如 cutoff–res、wet–pan），形成跨章节周期性“锚点”与结构重复性。
- 无理比相位：α/β 取无理比（φ、√2 等），保证长周期不整齐重复但总体回到锚点；确保仅作用于参数层，不破坏节拍量化。
- 平滑与边界：所有自动化包络带 T60 渐变与限幅，避免泵音与突变。

## 6. 确定性伪随机与模式库

- 无理步进：对模式索引/力度/装饰触发使用无理数步进的取整/取模（例如 frac(n·φ)→量化到离散集合）实现“伪随机”而可复现。
...
- 分层淡入：非鼓类片段均采用包络淡入（10–40 击拍范围），先频域后能量（先开滤再提电平）。
- 等功率交叉淡入：片段/章节在 A/B 总线进行 equal-power crossfade；鼓层保持恒定锚点。
- 频域交接：A：HPF 上行，B：LPF 下行；交接中心频率与时长随能量曲线确定。
...
3) 选择本 README 第 7 节中的非鼓片段给各章定调，并按第 8 节规则淡入。  
