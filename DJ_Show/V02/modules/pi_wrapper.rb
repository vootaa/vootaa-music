...
class PIWrapper
  ...
  def ambient(root = :a2, chord_type = :minor, bars = 8, fade_bars = 4)
    live_loop :pi_ambient, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      
      if bar < fade_bars
        fade_vol = bar.to_f / fade_bars
  ...
  def ocean(bars = 8, fade_bars = 4)
    live_loop :pi_ocean, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      
      if bar < bars
        fade_vol = bar < fade_bars ? (bar.to_f / fade_bars) : 1.0
        
        with_fx :lpf, cutoff: 80 + 40 * Math.sin(bar * 0.5) do
  ...
  def ambient_experiment(root = :a3, scale_type = :minor, bars = 4)
    live_loop :pi_ambient_exp, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      
      if bar < bars
        notes = scale(root, scale_type)
        pattern = [0, 2, 4, 8]  # 1-3-5-9
  ...
  def blimp_zones(root = :a2, scale_type = :minor, bars = 8)
    live_loop :pi_blimp, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
  ...
  def bach(root = :a2, scale_type = :minor, bars = 4)
    live_loop :pi_bach, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      ...
      if bar < bars
        ...
        with_fx :reverb, room: 0.8, mix: 0.7 do
          degrees.each do |deg|
            ...
            sleep 1.0
            play scale_notes[deg - 1], release: 0.5, amp: 0.2  # 邻接音
            sleep 0.5
            play scale_notes[deg], release: 1.5, amp: 0.35
            sleep 2.5
          end
  ...
  def reich_phase(root = :a4, bars = 8)
    live_loop :pi_reich, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
        ...
        2.times do |voice|
          in_thread do
            16.times do |i|
              ...
              sleep 0.25 + voice * 0.01 * bar  # 相位逐渐漂移
            end
          end
        end
        sleep 4
  ...
  def acid(root = :a2, bars = 8, cutoff_start = 60, cutoff_end = 110)
    live_loop :pi_acid, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      
      if bar < bars
        cutoff = cutoff_start + (cutoff_end - cutoff_start) * (bar.to_f / bars)
  ...
  def blip_rhythm(root = :a3, scale_type = :minor_pentatonic, bars = 4)
    live_loop :pi_blip, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      
      if bar < bars
        notes = scale(root, scale_type).take(8)
        pattern = [0, 2, 4, 7]  # 1-3-5-8
  ...
  def driving_pulse(root = :d2, bars = 8)
    live_loop :pi_pulse, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
      ...
      if bar < bars
        ...
        with_fx :reverb, room: 0.3 do
          4.times do
            ...
            sleep 0.5
            play root + 7, release: 0.8, amp: 0.5  # 五度
            sleep 0.5
          end
        end
      else
  ...
  def wob_rhyth(root = :a1, bars = 4)
    live_loop :pi_wob, sync: :bar_tick do
      bar_data = sync :bar_tick
      bar = bar_data[:bar]
