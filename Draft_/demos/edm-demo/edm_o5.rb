...
live_loop :cg do
  ...
  if [6, 14].include?(t % 16)
    ...
    if t % 16 == 8 && f > 0.6
      with_fx :reverb, room: 0.4 do
        sample :sn_dub, amp: M * 0.4, rate: 1.2
      end
    end
  end
  cr = [true, false, true, false, true, false, false, true, false, true, false, false, true, false, true, false]
  if cr[t % 16]
  ...
  if spread(3, 16)[t % 16] && M > 0.5
    sample :drum_cowbell, amp: M * 0.3, lpf: 120, rate: 2
  end
  if s == :edm
    if t % 1 == 0
...
live_loop :cm, sync: :cg do
  ...
  if t % 32 == 0 && t > 0
    sample :perc_bell2, amp: 0.2, lpf: 100, start: 0.05
  end
  if t % 64 == 0 && t > 0
    in_thread do
      6.times do
        sample :hat_zap, amp: 0.1
        sleep 0.25 / 3
      end
      sample :hat_zap, amp: 0.1
      sleep 0.25
...
# === 音乐层次结构 ===
# 🥁 节拍层(cosmic_genesis)：
#    - 宇宙心跳：量子态驱动的底鼓，带变奏和立体声钟摆效果
#    - 宇宙呼吸：宏观态控制的军鼓，带clap变体，椭圆轨道移动
#    - 量子脉冲：自定义16拍节奏的细节打击乐，螺旋轨道分布
#    - EDM hi-hat：每拍closed hi-hat，增加节奏密度
#    - Deep House shaker：循环compus样本，提供groove
#    - Cowbell：off-beat cowbell，增强groove
...
#    - 相位转换音效：每80拍的相位变化提示音
#    - 实时状态监控：显示当前相位和演化程度
#    - SFX事件：每32拍bell，每64拍hat序列，增加动态事件
...
# • 代码精简：290行实现13层音乐纹理和7种立体声轨道
# • 多风格支持：通过简单变量切换EDM/Deep House风格
# • 节奏增强：自定义spread + 变奏kick/clap/cowbell
# • SFX添加：动态事件样本，提升互动性
