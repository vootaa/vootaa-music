...
live_loop :kick do
  ...
  pan = LANE_PAN_EL.call(t) + VEL_PAN_OFF_EL * fusion
...
live_loop :melody do
  ...
  pan = HORIZON_PAN_EL + LANE_PAN_EL.call(t) * 0.2
...
live_loop :fx do
  ...
  pan = HORIZON_PAN_EL + rand(-0.4..0.4) * fusion
...
live_loop :events do
  t = current_beat * (60.0 / BPM_EL)
  fusion = get_fusion_el(t) + drift
  if fusion > 0.8 && event_subsets[variant_index]
    event = event_subsets[variant_index].sample
    case event
    when :bd_haus
      sample :bd_haus, amp: 0.6, pan: rand(-0.5..0.5)
    when :sn_dub
      sample :sn_dub, amp: 0.5, pan: rand(-0.5..0.5)
    when :synth_saw
      synth :saw, note: :e4, amp: 0.7, pan: HORIZON_PAN_EL
    when :fx_reverb
      with_fx :reverb do
        synth :saw, note: :g4, amp: 0.4, release: 1.5
      end
    when :amen_fill
      sample AMEN_POOL.sample, amp: 0.7, pan: rand(-0.3..0.3), rate: 1.0
...
live_loop :variant_ctrl do
  # Variant start prompt: unique Synth melody for Progressive Trance with stereo surround
  melody_notes = [:c4, :d4, :e4, :f4, :g4]  # Slow ascending melody for highway progression
  melody_notes.each_with_index do |n, i|
    pan = Math.sin(i * PI / 4) * 0.7  # Stereo surround: gradual left-right sweep
    synth :saw, note: n, amp: 0.5 + i * 0.05, release: 0.5, pan: pan  # Saw with gradual amp increase
  ...
  variant_index += 1
  drift = Math.sin(variant_index * PI / VARIANT_COUNT_EL) * 0.1
