...
- 外部采样：项目开发进行到后期，可以在后半段变体乐曲，引入由Python编码生成特定的MIDI，并用特定音色渲染生成的WAV文件。

- 关于变体：
-- 每个变体周期的开始部分，引入一个标志性事件，形成一个提示，比如选择适合乐曲风格的独特Synth音符旋律，适配乐曲风格的长度，并配合立体声环绕，音量需要跟上下文音量适配，避免突兀。
-- 变体中的提示，建议从SONIC PI库获取，不要跟乐曲的其它元素冲突，采用Fade In的方式，否则会显得突兀。
-- 每个变体之间引入间隙，使得乐曲具有呼吸感。
-- 满足变体数量后，乐曲应该自动停止，而不是无限循环播放。
-- 每个变体乐曲单元中，从事件池中获取提供独特的事件序列，比如FX，Sample，合成器，效果器等。
-- 事件池中的事件依次分配到不同的变体中，确保每个变体拥有不同的事件序列（不同的特效，Sample，合成器，效果器，及其不同的参数配置），目的是让车载乐曲引入丰富的变化特色，避免单调性。
...
- **数学序列律动**：用无理数（如π）小数位生成 *伪随机序列*（整个项目中不能采用随机数），驱动节奏/音高变化，确保可复现微妙演变；实现：序列映射到音符索引，结合fusion阈值调整播放概率。
...
- DEBUG开关控制：在DEBUG状态下，整个乐曲时间缩短为1/5，这样可以快速体验完整个乐曲的效果，加速开发阶段的测试。
...
- 数学驱动的律动创新，但旋律可能过于抽象，缺乏情感深度或和声丰富性。
-- 和声增强：不同的变体，可以采用多个和弦进行，基于黄金比例选择音符，避免单调。示例：在旋律中添加chord_degree选择。
-- 节奏多样：结合多轨道（如鼓+旋律），使用PI驱动节奏变体，增加紧张感。
-- 动态平衡：添加“呼吸”机制，如在变体间插入静默或渐弱，模拟驾驶节奏。
-- 风格融合：在乐曲中间部分的变体中（排除首尾变体单元），混合其它风格元素（如Trance融入Ambient）。
