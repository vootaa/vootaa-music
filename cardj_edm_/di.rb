...
define :pe do |time, frequency = 1.0|
...
define :dt do |energy_level|
  {
    ...
    cp: [chord(:c4, :M7),chord(:f4, :M7),chord(:g4, :dom7),chord(:c4, :M7)],
...
define :dawn_fusion_controller do |ultra_time|
  ...
  stage = [[stage_raw, 0].max, 4].min  # 替代 clamp(0, 4)
  ...
  current_config[:energy] += pe(ultra_time * 20) * 0.1
  current_config[:micro_variations] = pi_seq((ultra_time * 1000).to_i)
...
live_loop :dawn_master_controller do
  ...
  techniques = dt(fusion_data[1][:energy])
...
live_loop :dawn_kick_fusion, sync: :dawn_master_controller do
  ...
  coords = time_coordinates(t)
  mathematical_intensity = pe(coords[:micro] * 4)
  ...
  when :gentle_morning
    ...
    base_trigger || (math_mod && pi_seq(t/4) > 0.8)
    
  when :light_bounce
    (t % 4 == 0) || (t % 4 == 2 && pi_seq(t/8) > 0.7)
    
  when :steady_four, :house_classic, :full_drive
    complexity = config[:energy]
    base_hits = [0, 2].include?(t % 4)
    extra_hits = complexity > 0.5 && [1, 3].include?(t % 4) && pi_seq(t) > (1 - complexity)
    base_hits || extra_hits
  end
  
  if should_kick
    amp_base = config[:techniques][:gentle_kick][:amp_base]
    amp_variation = pi_seq(t, 3) * 0.3
    
    # 修复 cutoff 计算，确保在有效范围内
    cutoff_val = [[60 + config[:energy] * 50, 130].min, 20].max
...
live_loop :dawn_melody_fusion, sync: :dawn_master_controller do
  ...
  if config[:energy] > 0.4
    ...
    current_chord = config[:techniques][:cp][(coords[:macro]/4).to_i % 4]
    ...
    fractal_offset = (pi_seq(t, 11) - 0.5) * 7
    final_note = base_note + fractal_offset
    
    timing_humanize = (pi_seq(t, 13) - 0.5) * 0.1
    ...
    # 修复 sleep 计算，确保不会是 nil
    sleep_choices = [0.25, 0.5, 0.75]
    sleep_index = (pi_seq(t, 17) * 3).to_i % sleep_choices.length
    base_sleep = sleep_choices[sleep_index]
    
    # 确保 timing_humanize 不会让 sleep 变成负数或 nil
    final_sleep = [base_sleep + timing_humanize, 0.1].max
    sleep final_sleep
  else
    sleep 2
  end
...
live_loop :dawn_ambient_fusion, sync: :dawn_master_controller do
  ...
  phi_note_offset = (pe(coords[:ultra] * 50) * 24).round
  ambient_note = :c5 + phi_note_offset
  
  reverb_room = 0.8 + pi_seq(t/64) * 0.2
  reverb_mix = config[:reverb_space] + (pi_seq(t/32, 5) - 0.5) * 0.2
  
  with_fx :reverb, room: reverb_room, mix: reverb_mix do
    with_fx :echo, phase: pe(coords[:ultra] * 30) * 0.5 + 0.25 do
      play ambient_note,
        attack: 3 + config[:energy] * 2,
        release: 12 - config[:energy] * 4,
        amp: 0.1 + (1 - config[:energy]) * 0.2,
        cutoff: 60 + pi_seq(t/16) * 30,
        pan: (pi_seq(t/8, 9) - 0.5) * 0.8
    end
  end
  
  # 修复 sleep 计算
  base_sleep = 8
  sleep_variation = pi_seq(t/4) * 4
  final_sleep = [base_sleep + sleep_variation, 1].max
  sleep final_sleep
end
...
live_loop :dawn_bass_fusion, sync: :dawn_master_controller do
  ...
  if config[:energy] > 0.35
    ...
    if note_to_play != 0
      if pi_seq(t, 19) < (config[:energy] * 0.8 + 0.2)
        play note_to_play,
