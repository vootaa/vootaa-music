...
when :algorithmic_techno then {room: 0.4, phase: 0.2, decay: 4, amp: 0.25, max: 3, synth: :tb303, chord: :minor, bpm: 128, fx: :distortion}
when :quantum_wave then {room: 0.3, phase: 0.1, decay: 2, amp: 0.2, max: 4, synth: :fm, chord: :major, bpm: 110, fx: :reverb}  # 修复：:chorus改为:reverb
when :fractal_rhythm then {room: 0.5, phase: 0.3, decay: 3, amp: 0.15, max: 5, synth: :saw, chord: :minor7, bpm: 120, fx: :echo}  # 修复：:flanger改为:echo
...
define :play_electronic_chord do |chord_scale, amp=0.2, attack=0.5, release=1.5, tick_offset=0|
  ...
  chord_notes = chord(root, ELECTRONIC_CONFIG[:chord]).take(ELECTRONIC_CONFIG[:max])
  ...
  with_fx :echo, phase: ELECTRONIC_CONFIG[:phase], decay: ELECTRONIC_CONFIG[:decay] do
    with_fx ELECTRONIC_CONFIG[:fx], mix: 0.3 do
      ...
      live_loop :electronic_evolution do
        ...
        play chord_notes.choose, amp: 0.3, attack: 0.1, release: 1.0,
      ...
      # 主干：鼓点（固定4/4模式，辅助偶尔变异）
      live_loop :electronic_drums, sync: :electronic_evolution do
        evolution_tick = look
        pattern = DRUM_PATTERNS[:basic]  # 固定模式
        
        8.times do |i|
          sample :drum_bass_hard, amp: 0.5 if pattern[i] == 1
          sample :drum_snare_hard, amp: 0.4 if (i % 4) == 2
      ...
      live_loop :electronic_arpeggios, sync: :electronic_evolution do
        ...
        use_synth :saw
        play_electronic_chord(chord_scale, 0.15, 0.05, 0.8, evolution_tick)
        sleep 4  # 主干：固定4拍
      end
      ...
      live_loop :electronic_bassline, sync: :electronic_evolution do
        ...
        use_synth ELECTRONIC_CONFIG[:synth]
        play bass_note, amp: 0.4, release: 1.5,
      ...
      live_loop :electronic_variations, sync: :electronic_evolution do
        ...
        if electronic_rand(0, 1, :golden, evolution_tick) > 0.8  # 仅偶尔
          ...
          use_synth :beep
          play variation_note, amp: 0.1, release: 1, cutoff: 80 + electronic_rand(-10, 10, :fine, evolution_tick)
        end
        sleep 4  # 匹配主干
      end
    end
