...
define :play_cosmic_layer do |layer_type, t, intensity, phase_notes|
  ...
  when :particle
    ...
    particle_note = phase_notes.sample + [12, 24, 36].sample
...
live_loop :cosmic_genesis do
  ...
  # 优化：减少spread计算，简化sample选择
  if spread(5, 16)[t % 16]
    sample [:perc_bell, :elec_tick, :elec_blip2, :perc_snap, :elec_pop, :ambi_glass_rub, :ambi_piano, :ambi_soft_buzz, :ambi_swoosh].choose,
      amp: fusion * 0.4,
    ...
    in_thread do
      3.times do |i|
        ...
        play chord(phase_notes[i % phase_notes.length], chord_type),
          amp: fusion * 0.2, attack: limit_range(1.5 + i * 0.5, 0, 4),
  ...
  if t % 8 == 0 and fusion > 0.6
    ...
    play note(phase_notes[0]) - 24, amp: macro * 0.6, attack: 0.1, release: 2,
