...
live_loop :cg do
  ...
  if t % 32 == 0
    in_thread do
      3.times do |i|
        use_synth mp([:hollow, :prophet, :saw, :blade], :e)
        play chord(pn[i % pn.length], mp(S_CHD, :golden)),
...
#    - wave：幅值叠加波包 → 类潮汐起伏。
#    - random：黄金数字序列驱动的伪随机定位（粒子/瞬态用），兼顾稀疏扩散与可复现性。
