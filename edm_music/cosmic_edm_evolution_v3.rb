...
define :mr do |a=0.0,b=1.0,c=:pi,adv=1|
  @md_seq ||= {}
  seq = @md_seq[c] ||= (MD[c] || MD[:pi]).chars.map(&:to_i)
  l = [seq.length - 1, 1].max
  i = @dc[c] % l
  digits = seq[i, 2] || [seq[i]]
  val = digits.reduce(0) { |acc, d| acc * 10 + d }
  scl = (10**digits.length) - 1
  @dc[c] = (@dc[c] + adv) % l
  a + (val / scl.to_f) * (b - a)
end
...
define(:phs){|t|
  base = EP[((t*0.01)%EP.length).to_i]
  if defined?(PH_R) && PH_R[base] && qs(t*0.125, :macro) > 0.7
    base = PH_R[base][mr(0, PH_R[base].length, :golden).to_i]
  end
  base
}
...
define :sw? do |t=nil|
  t ||= get(:gt)||0
  (t%512).between?(256,271)
end
define :sf do
  bd_vt=get(:bd_vt)||-10
  age=vt-bd_vt
  age>0.15 ? 1.0 : 0.4+age/0.15*0.6
end
define(:fxp){|f| f<0.4 ? [] : (f>0.7 ? S_FX_HI : S_FX)}
define :tfx do |t,f|
  if t%64==60 && f>0.5
    sample mp(S_FXT,:golden),amp:f*0.35,rate:lr(0.8+f*0.4,0.5,1.5),pan:cp(t,:wave)
  elsif t%128==124 && f>0.5
    sample mp(S_FXT,:pi),amp:f*0.45,rate:lr(0.6+f*0.5,0.4,1.4),pan:cp(t*0.5,:galaxy)
  end
end
define :es do |i,nm,role=:auto,e=nil|
  ...
  if defined?($ph) && PH_E[$ph]
    env = PH_E[$ph]
    set :f_atk, (get(:f_atk) || 1) * env[:atk]
    set :f_rel, (get(:f_rel) || 1) * env[:rel]
  end
  sc
end
define :plr do |ch,amp,cut,pan,ma,ml|
  ...
  play r-12,amp:ar,attack:tr,release:rr,cutoff:cr,pan:pj(pan*0.6,0.1)
  md.each{|n| play n,amp:am,attack:tm,release:rm,cutoff:cm,pan:pj(pan,0.05)}
...
define :pl do |k,t,i,pn|
  ...
  when :harmonic
    if t%3==0 && i>0.4
      n=pn[t%pn.length]+12; ln=get(:lead_note); n+=5 if ln && note(ln)==note(n)
      es(i,n,:accent,i)
      play n,amp:ka(i*0.3),mod_range:lr(i*12,0,24),mod_rate:lr(i*8,0.1,16),attack:kt(0.1),release:kr(1.5),pan:pj(cp(t,:figure8))
    end
  when :tremolo
    if spread(7,32)[t%32]
      n=mp(pn,:e)+mp([0,7],:golden); es(i,n,:arp,i)
      with_fx(:tremolo,phase:lr(i*2,0.1,4),mix:lr(i*0.8,0,1)){ play n,amp:ka(i*0.4),cutoff:lr(kc(60+i*40),20,130),release:kr(0.8),pan:pj(cp(t,:wave)) }
    end
  when :particle
    ml=get(:mute_lead); tl=ml ? 0.68 : 0.72; th=ml ? 0.92 : 0.90
    if i>tl && i<th
      n=mp(pn,:golden)+mp([12,24,36],:e); es(i,n,:accent,i)
      with_fx(:reverb,room:0.4,mix:0.3){ play n,amp:ka(i*0.2),attack:kt(0.05),release:kr(0.3),cutoff:lr(kc(80+i*30),50,130),pan:pj(cp(t+mr(0,16,:pi),mp(S_PAN2,:golden))) }
    end
  end
end
live_loop :cg do
...
live_loop :cm, sync: :cg do
  ...
  if t%8==0
    ...
    with_fx(:lpf,cutoff:(sw? ? 70 : lr(40+ai*20,20,130))) do
      play pn.map{|n|note(n)-36}.take(3),amp:ai*0.15,attack:8,release:16,pan:cp(t*4,:galaxy)*0.6
    end
  end
  if t%20==0 && t>0
    use_synth :prophet
    with_fx(:reverb,room:0.8,mix:0.6){ with_fx(:echo,phase:0.375,decay:2){ pn.each_with_index{|nv,i| at(i*0.2){ play nv+12,amp:0.3,attack:0.5,release:2,cutoff:90,pan:cp(t*4+i*4,:spiral) }}} }
  end
  sleep 4
end
